# Phase 2 受け入れ基準 セルフチェック (2026-07-12 現況追記)
...
>
> **2026-07-12 現況**: QuestPDFライセンス、日本語フォント、KouchinModule v5突合、請求用平均工賃の正式定義は解消済み。Phase 2の`AverageWageMetric`は暫定集計として互換保持し、請求用の正式計算はADR 0023に従うPhase 3-1 Task 15で別実装する。
...
| **AC2-8** 平均工賃月額メトリクス (暫定式) | [~] partial | A1 (ADR) / B10 | `AverageWageMetricTests` (3) — Phase 2の暫定集計として互換保持。請求用正式式はADR 0023で確定し、Phase 3-1 Task 15で別実装 |
...
- **Phase 2受け入れ時のテスト総数**: 469 (Domain 146 / Application 96 / Infrastructure 119 / Reporting 5 / App 103)
- **Phase 2受け入れ時の品質ゲート**: build 0 warnings / format clean / Architecture / Offline (Reporting も対象) / AppOffline 全 pass
- **Phase 2受け入れ時のカバレッジ**: Domain **98.03% line** (`build/ci.sh` gate **≥95%**) / Application ≥ 70% floor
...
## Phase 2 deferred の現況 (2026-07-12)

| 種別 | 現況 | 根拠 |
|------|------|------|
| **License** | ✅ 解消 — QuestPDF Community Licenseの適用条件と再判定条件を確定 | ADR 0013 / Phase 4 S1 |
| **Font** | ✅ 解消 — Noto Sans JPを埋め込み、環境フォント依存を停止 | Phase 4 S1 / `CHANGELOG.md` |
| **Spec** | ✅ 解消 — KouchinModule.bas v5を突合しADR 0012を正式化 | ADR 0012 / Phase 4 S0 |
| **Spec** | ✅ 定義確定 — 請求用平均工賃の正式式を確定。Phase 2指標は互換保持し、請求用実装はPhase 3-1 Task 15へ分離 | ADR 0023 |

> 2026-06-29 解消: 「WageStatementView の SaveFileDialog 配線」は M-2 でクローズ
> (commit `eb6b3ad..d50e53a`)。

## Phase 1 final review からの繰越 deferred 現況

- [x] RecipientEditView 到達性 (Enter binding / タブ追加 / UpdateRecipient 配線)
- [x] DailyRecord 月選択 UI
- DailyRecord per-cell Space/Backspace/F2 keyboard nav (custom UserControl 必要)
- [x] DailyRecord訂正UI（既存記録は統一コマンドから`CorrectDailyRecordUseCase`へ分岐）
...
AC2-8の正式定義はADR 0023で解消済み。Phase 2の暫定集計指標は互換保持し、請求用実装と残るUI deferredは各現行計画で追跡する。
