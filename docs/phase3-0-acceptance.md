...
## Deferred / 非実装（Phase 3-0受け入れ時点）

- Phase 3-1〜3-3、請求報酬計算、validated finalization、`IValidatedClaimSnapshotReader`、3帳票生成、CP932/CRLFの提出CSV生成、保存UIは未実装だった。現在の進捗は上記「受け入れ後の現況」を参照する。
- Phase 3-1は51 missing fieldIdsを26 implementation targetsへ束ね、モデル・migration・実UIを追加してからvalidated readerとproduction codecを実装する計画とした。自由記述、既存別用途項目、0、空文字から推測しない。
- Phase 3-2 / 3-3は、Phase 3-1で検証された実効`ClaimBatch`だけを入力にする。
- 2026-06-29付のPhase 3旧計画は実行対象外。後続の現行計画は [`docs/superpowers/plans/2026-07-11-phase3-1-claim-calculation-and-input-foundation.md`](superpowers/plans/2026-07-11-phase3-1-claim-calculation-and-input-foundation.md) として作成済み（その後2026-07-19に [`2026-07-19-phase3-1-minimal-vertical-slice.md`](superpowers/plans/2026-07-19-phase3-1-minimal-vertical-slice.md) へ置換。上記「受け入れ後の現況」参照）。
