...
- count selectorが後続calculatorへ解決するruntime fact値の検証。
...
- 後続calculatorはvalidator済みのtyped unionだけを受け取り、生JSON又は自由記述式を解釈しない。count selectorをruntime factへ解決し、0以下のcount、0除算相当及びrecipient上限超過を算定不能として拒否する責務もここに置く。
