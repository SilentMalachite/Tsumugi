...
1. **Audit gate:** 41資料を再取得してSHA-256を検証し、既存14,709行のsource-side manifestをschema v2へ機械変換する。別candidateで公式割合根拠3頁を追加した補正後14,712行を、一次資料に対して再監査する。
...
- manifestはschema v2へ機械変換済みである。source inventory補正後は41 documents、53 ranges、14,712 rowsで、再監査前の`schema-gap`は13,953 rowsである。
...
SHA検証済み原本に対して補正後の全14,712行を再監査し、各rowを次へ分類する。
...
- manifest range数が53、row数が14,712。
- range別`expectedItemCount`合計とrow数が一致する。
- v1の14,709 row identitiesが順序を保って存在し、承認済み3 identities以外の追加がない。
- ordered identity SHA-256が`76f0e7e13c52061c9190948da171df108afee38bb9f525951d535d1eb0ba8f18`と一致する。
...
- 41 documents、53 ranges、14,712 rows及び承認済みsource inventory correction。
...
- 41 documents、53 ranges、14,712 source rows及び全rangeがmanifest v2へ保持されている。
