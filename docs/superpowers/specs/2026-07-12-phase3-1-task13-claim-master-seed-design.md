...
1. **Audit gate:** 41資料を再取得してSHA-256を検証し、既存14,709行のsource-side manifestをschema v2へ機械変換する。公式割合根拠3頁を追加した14,712行を経て、R6報酬算定構造36頁の期間別根拠3 logical rowsを追加した補正後14,715行を、一次資料に対して再監査する。
...
- manifestはschema v2へ機械変換済みである。source inventory補正後は41 documents、53 ranges、14,715 rowsで、再監査前の`schema-gap`は13,956 rowsである。
...
row identityは`sourceDocumentId + rangeId + sourceLocator`とする。v1からv2への機械移行ではrowの追加、削除、重複、並べ替えを行わない。source inventory補正では既存14,709 identitiesの順序を保持し、承認済み3 identitiesだけを各documentの既存row直後へ追加する。v1の`masterKind`、`seedKey`、`aggregationId`、`aggregationKind`及び`aggregationReason`はrow直下から削除し、v2では禁止する。
...
SHA検証済み原本に対して補正後の全14,715行を再監査し、各rowを次へ分類する。
...
- manifest range数が53、row数が14,715。
- range別`expectedItemCount`合計とrow数が一致する。
- v1の14,709 row identitiesが順序を保って存在し、承認済み3 identities以外の追加がない。
- ordered identity SHA-256が`e350d208f9862fa2a4a0328df7e3ad6bca145b5e8dfde38a63a0c4d79126d225`と一致する。
...
- 41 documents、53 ranges、14,715 rows及び承認済みsource inventory correction。
...
- 41 documents、53 ranges、14,715 source rows及び全rangeがmanifest v2へ保持されている。
