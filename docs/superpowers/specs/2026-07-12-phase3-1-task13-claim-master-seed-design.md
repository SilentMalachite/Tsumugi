...
**Status:** Spec review approved; source inventory correction approved
**Date:** 2026-07-14
...
1. **Audit gate:** 既存14,718 identitiesを順序付き部分列として保持し、基準該当B型の3資料、8 period-specific evidence rows及び44 source rowsのclosed mappingを加えた44 documents、61 ranges、14,726 rowsを一次資料に対して再監査する。current ordered identity SHA-256は`c80f4e8da0aefc9d91bd978777bdb8e59261f4982826555f8a324e2023b9bcd7`に固定する。
...
- manifestはschema v2へ機械変換・再監査済みである。基準該当B型follow-up後のcurrent inventoryは44 documents、61 ranges、14,726 rows、14,189 seed、537 excluded、0 schema-gapである。6 production seedは空のままであり、保護施設事務費の実値seed、resolver及びruntime算定は未実装である。
...
取得対象は現行manifestの44 documentsに固定する。各document ID、URL及び期待SHA-256は`src/Tsumugi.Infrastructure/ClaimMasters/Seed/sources.json`を正本とし、Task 13で別URL又は別SHAへ置き換えない。
...
`documents`はv1のdocument ID、SHA、role及び`extractionRanges`を保持する。document順序、range順序及びrangeの`expectedItemCount`を機械移行又は再監査で変更しない。一次資料から既存inventoryの誤りを確認した場合はTask 13を停止し、必要なrange／row修正を別candidateとして設計し直す。同じcandidate内で件数を直してgateを通さない。

次の3段落は本follow-up以前の承認済みcandidate履歴であり、current audit gateではない。

2026-07-14の別candidateでは、再監査で確認したsource-range gapを解消するため、`r6-fee-notice`物理141–142頁と`r8-fee-notice`物理56頁を2 ranges・3 rowsとして追加する。補正後inventoryは41 documents、53 ranges、14,712 rows、ordered identity SHA-256は`76f0e7e13c52061c9190948da171df108afee38bb9f525951d535d1eb0ba8f18`に固定する。3 rowsは再監査前のため`schema-gap`とし、このcandidateではproduction targetへ写像しない。

同日の後続candidateでは、再監査で確認したR6処遇改善割合の期間別根拠不足を解消するため、`r6-reward-structure`物理36頁を2024-04..2024-05、2024-06..2025-03、2024-06..2026-05の3 logical rowsとして既存rangeへ追加する。補正後inventoryは41 documents、53 ranges、14,715 rows、ordered identity SHA-256は`e350d208f9862fa2a4a0328df7e3ad6bca145b5e8dfde38a63a0c4d79126d225`に固定する。既存14,712 identitiesの順序を保持し、3 rowsは再監査前のため`schema-gap`とする。

さらに後続candidateでは、上記3期間のservice code側に必要な算定対象・計算段階・四捨五入根拠を期間一致で対応させるため、`r6-calculation-note`物理8頁を同じ3期間のlogical rowsとして既存rangeへ追加する。補正後inventoryは41 documents、53 ranges、14,718 rows、ordered identity SHA-256は`90fb9d309e878d22f0d4bb867c4fe36c3fab83ad45938b64da2d5b3bfd34dee7`に固定する。既存14,715 identitiesの順序を保持し、3 rowsは再監査前のため`schema-gap`とする。
...
row identityは`sourceDocumentId + rangeId + sourceLocator`とする。v1からv2への機械移行ではrowの追加、削除、重複、並べ替えを行わない。初期source inventory補正では既存14,709 identitiesの順序を保持し、承認済み9 identitiesだけを各documentの既存row直後へ追加した。基準該当B型follow-upでは、その14,718 identitiesを新manifest identitiesの順序付き部分列として保持し、8 period-specific evidence identitiesだけを末尾へ追加する。v1の`masterKind`、`seedKey`、`aggregationId`、`aggregationKind`及び`aggregationReason`はrow直下から削除し、v2では禁止する。
...
### 7.4 production revision identity

各production targetは親manifest rowの`effectiveFrom`と`effectiveTo`を継承する。target revision identityを次の4-tupleに固定する。

```text
(masterKind, seedKey, effectiveFrom, effectiveTo)
```

production側のentry又はcondition revisionも同じ4-tupleで識別する。`effectiveTo = null`は上限なしとして値を変換せず比較する。

- 1つのsource rowが複数targetへ対応する場合、targetごとに4-tupleを作る。
- 複数source rowが同じproduction revisionを裏付ける場合、同じ4-tupleを参照する。
- 同じ`masterKind + seedKey`でも期間が異なるproduction revisionは別targetとして扱う。
- `primary`又は`component` targetの親row期間はproduction revision期間と完全一致させる。
- `supporting-evidence` targetも、裏付けるproduction revisionの期間を親rowへ明示し、暗黙に全期間へ拡張しない。

完全性検査は次の双方向照合とする。

1. manifestのdistinct target revision identityごとに、一致するproduction revisionがちょうど1件存在する。
2. production revisionごとに、一致するmanifest target revision identityが1件以上存在する。
3. manifest targetを持つ各source rowについて、同一production revisionに`documentId + locator + supports[]`が一致するsource refがちょうど1件存在する。
4. production revisionの各source refについて、対応するmanifest source rowとtargetが1件以上存在する。

この双方向照合により、同じkeyの後続版欠落、誤った期間へのsource ref接続及びmanifestに根拠のないproduction revisionを拒否する。
...
1 source rowから複数production entryへのprojectionは複数targetで表す。複数source rowから1 production revisionへの集約は同じtarget revision identityを参照する。source row数とproduction row数の単純一致は要求せず、distinct target revision identity集合とproduction revision identity集合を双方向比較する。
...
SHA検証済み原本に対してcurrent inventoryの全14,726行を再監査し、各rowを次へ分類する。
...
- service code identity、selector、condition及び次のunit rule。
  ...
  - `formula / protected-facility-benchmark-minimum`
- 対象外row。
...
- manifest document数が44。
- manifest range数が61、row数が14,726。
- range別`expectedItemCount`合計とrow数が一致する。
- 既存14,718 row identitiesが順序付き部分列として存在し、8 period-specific evidence identities以外の追加がない。
- ordered identity SHA-256が`c80f4e8da0aefc9d91bd978777bdb8e59261f4982826555f8a324e2023b9bcd7`と一致する。
- 取得率、SHA一致率、locator到達率が100%。
- 全rowのdisposition契約が有効。
- dispositionが14,189 seed、537 excluded、0 schema-gapで、合計14,726と一致する。
- `schema-gap = 0`。
...
manifestのdistinct production target revision identity集合と、6 seedのentry及びcondition revision identity集合を双方向比較するcompleteness testを先に追加する。audit gate直後はseedが空であるため、testが期待した理由でRedになることを確認する。
...
- 44 documents、61 ranges、14,726 rows、14,189 seed、537 excluded、0 schema-gap。
- 既存14,718 identitiesの順序付き部分列保存及びordered identity SHA-256 `c80f4e8da0aefc9d91bd978777bdb8e59261f4982826555f8a324e2023b9bcd7`。
...
- manifest target revision identity集合とproduction entry／condition revision identity集合の双方向完全一致。
- manifest targetの全source rowとproduction source refの双方向完全一致。
...
- v1／v2でrow identity又はrange countが変化する。
...
- 44 documents、61 ranges、14,726 source rows及び全rangeがmanifest v2へ保持されている。
- 14,189 seed、537 excluded、0 schema-gapで、既存14,718 identitiesが順序付き部分列として保持されている。
- ordered identity SHA-256が`c80f4e8da0aefc9d91bd978777bdb8e59261f4982826555f8a324e2023b9bcd7`と一致する。
- 取得率、SHA一致率、locator到達率、manifest coverageが100%。
- `schema-gap = 0`。
- manifest target revision identity集合とproduction entry／condition revision identity集合が双方向に完全一致する。
- manifest targetの全source rowとproduction source refが双方向に完全一致する。
