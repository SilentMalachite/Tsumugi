...
- URL: https://www.questpdf.com/license/ （詳細: https://www.questpdf.com/license/community.html）
- 適用条件（一次情報抜粋）:
  - 年商閾値: *"businesses with less than USD 1,000,000 in annual revenue"* が Community License の対象。
  - 適用単位: **組織（法人）単位**。*"applies at the organization/business entity level, not per-project."*（個別事業所単体の年商ではなく、運営法人全体の年商が基準）
  - 非営利/学術: *"charitable organizations, academic institutions"* および *"qualifying non-profits"* が適用対象として明記。
...
- 判断: **Community 継続確定**。閾値は法人単位の年商 USD 1,000,000 未満であり、就労継続支援B型事業所（想定利用者＝小規模事業所〜小規模法人）の年商はこれを大きく下回るため整合する。
- 留意事項（将来の再確認ポイント）: 適用単位が「個別事業所」ではなく「法人単位」であるため、複数事業所（B型・A型・生活介護等）を運営し**法人全体の年商**が USD 1,000,000 に近づく中〜大規模法人が導入する場合は再判定が必要。ADR 0013 に「発動条件（法人単位年商 USD 1M 接近時）と Professional 移行／Avalonia 印刷経路フォールバックの実装計画」として明記する。
...
- 3 URL とも一次情報（またはその直下の一次サブページ／API）に到達し内容を確認済み。到達失敗なし。

### 総合判断

- QuestPDF: **Community 継続確定**（法人単位年商 USD 1,000,000 未満で適用、就労B型事業所の想定規模と整合）。
- OFL 1.1／Noto Sans JP: ライセンス・帰属要件・配布形式を確認済み。**ただし .otf の入手元は Google Fonts 既定パッケージではなく `notofonts/noto-cjk` の GitHub Releases を使うこと**を実装時の申し送り事項とする。
- ステータス: **DONE**（法人単位 vs 事業所単位の閾値適用差異、および .otf 入手元の申し送りは ADR 0013／次タスクへ転記）。
