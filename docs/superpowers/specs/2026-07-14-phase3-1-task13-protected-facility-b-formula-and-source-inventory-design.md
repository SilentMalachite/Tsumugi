...
ただし、Task 12設計15.4で旧shapeの代表fixtureとして固定した`r6-service-codes-2-xlsx / workbook-order=38;row=907 / 462841`及び`row=908 / 462842`は、どちらも本設計の対象44 rowsに含まれる。両fixtureを`protected-facility-benchmark-minimum`へ置き換え、旧`base-component-pass-through`／base component付き`factor-chain`として受理する既存testsを更新する。他のformula fixturesは変更しない。
...
- 6 production seed filesは変更しない。対象44 rowsの`productionTargets`は後続seed phaseで使用する計画revision identityとして保持し、本follow-upでは実在revisionとの双方向一致を主張しない。
...
- 各rowは後続seed phaseで作成する`service-codes` production revision identityへ`mappingRole = primary`で1件以上写像する。対応revisionの実在性は本follow-upのgateに含めない。
...
各evidence rowは親rowと同じ期間の22 planned production revision identitiesだけへ`supporting-evidence`で写像し、非空`mappingReason`を持つ。R6 rowをR8 identityへ、又はR8 rowをR6 identityへ写像しない。1 evidence rowから同一期間内の複数identityへの写像は既存manifest v2契約どおり複数`productionTargets[]`で表す。
...
- 44 rowsすべてが対応planned production revision identityへ`primary` mappingを持つ。
...
- manifest evidence mappingsのdocument ID、locator及びsupportsがsource catalog及びauthority区分と整合し、planned production target identitiesの期間が対応row期間と一致する。
...
6. 対象44 rowsの`productionTargets`を後続seed phase用のplanned revision identitiesとして固定し、実在revisionとの双方向照合は行わない。
...
- 44 schema-gap rowsが公式service code identityを保ったままplanned production revision identitiesへ写像される。
...
- manifest evidence mappings、planned production targets及びsource catalogのprovenanceが内部整合し、production revisionの実在性は主張しない。
...
本設計の実装完了後も、対象44 service-code revisionsを含むproduction seed転記、保護施設事務費の実値record、append-only revision、証拠取込、入力UI、通常B型基本報酬Ⅱの比較単位解決、地方公共団体条件解決及び公式計算のruntime実行は未実装である。これらは別Taskの設計承認を経て実装する。
