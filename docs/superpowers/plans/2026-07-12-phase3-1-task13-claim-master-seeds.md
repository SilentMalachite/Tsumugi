...
**Goal:** 41件の一次資料とsource inventory補正後14,718件のsource rowをschema v2 manifestへ損失なく再監査し、Audit Gate通過後だけR6／R8制度実値を6つのclaim master seedへ投入する。
...
### Task 4.6: R6処遇改善割合3期間をlogical source rowへ追加する
...
R6報酬算定構造36頁の既存broad row直後へ、次の3 logical rowsを追加する。

- `legacy-treatment-improvement`: 2024-04..2024-05
- `integrated-treatment-improvement-v`: 2024-06..2025-03
- `integrated-treatment-improvement-i-iv`: 2024-06..2026-05

補正後contractは41 documents、53 ranges、14,715 rows、ordered identity SHA-256 `e350d208f9862fa2a4a0328df7e3ad6bca145b5e8dfde38a63a0c4d79126d225`とする。既存14,712 identitiesは順序を保持したsubsequenceとし、新規3 rowsは再監査前なので`schema-gap`かつ空の`productionTargets`でなければならない。

---

### Task 4.7: R6処遇改善計算根拠3期間をlogical source rowへ追加する
...
R6算定留意事項8頁の既存broad row直後へ、Task 4.6と同じ3期間のlogical rowsを追加する。これらはservice code側の`unit-rule-target`、`unit-rule-step`、`unit-rule-rounding`を期間一致で裏付ける。

補正後contractは41 documents、53 ranges、14,718 rows、ordered identity SHA-256 `90fb9d309e878d22f0d4bb867c4fe36c3fab83ad45938b64da2d5b3bfd34dee7`とする。既存14,715 identitiesは順序を保持したsubsequenceとし、新規3 rowsは再監査前なので`schema-gap`かつ空の`productionTargets`でなければならない。

---

### Task 5: 14,718 source rowsをv2 targetへ再監査する
...
test "$(jq -s 'length' /tmp/tsumugi-phase31-task13/chunks/*.jsonl)" = 14718
```

Expected: 74 chunk files、合計14,718 rows。
...
Expected: 全14,718 identitiesにdecisionがちょうど1件ある場合だけexit 0。missing／extra／duplicateで停止する。
...
Expected for continuing: `total = 14718`、`schemaGap = 0`、`total = seed + excluded`、`targets > 0`。
...
Expected: `total = reachable = 14718`、`failures = []`。
...
Expected: jq exit 0、identity SHAは`90fb9d309e878d22f0d4bb867c4fe36c3fab83ad45938b64da2d5b3bfd34dee7`。
...
3. 53 rangesと14,718 source rowsのlocatorを確認。
...
Issues Foundなら該当manifest／seed／testを修正し、Task 13のfocused testsとfull CIを再実行して新`SEED_CANDIDATE`を作る。同じreviewerが全14,718 rowsを再レビューする。差分rowだけで承認しない。
...
4. fresh reviewerによる全14,718 rows再照合。
...
41 documents、53 ranges、14,718 row identitiesが保持されている。
