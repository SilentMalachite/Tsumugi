...
- [ ] **Step 1: 旧 Phase 3-2 計画に SUPERSEDED 追記**
...
- [ ] **Step 2: `git status --short` を実行してワークツリー確認**
...
- [ ] **Step 2: 失敗テストを書く（codec v2 identity + canonical bytes 決定論）**
...
- [ ] **Step 4: `ClaimSnapshotValidationCodecV2` を実装**
...
- [ ] **Step 7: `ClaimRecipientSnapshotWriter` の identity 定数を v2 参照へ差替**
...
- [ ] **Step 8: 全 test suite で v1 参照が残っていないことを確認**
...
- [ ] **Step 9: ビルド + 全 test 緑確認**

Run: `dotnet build -c Release && dotnet test -v minimal`
Expected: 警告ゼロ、全テスト緑

- [ ] **Step 10: 失敗テストを書く（finalization writer + parser round-trip）**
...
- [ ] **Step 12: `ClaimFinalizationSnapshot` DTO 定義**
...
- [ ] **Step 2: 失敗テストを書く（21フィールド全てが payload に載る）**
...
- [ ] **Step 4: `OperationLocalSnapshotReader` を実装**
...
- [ ] **Step 7: 既存 `CloseClaimUseCase` テストを緑に戻す**
...
- [ ] **Step 1: 失敗テストを書く（ViewModel binding × 10 fields）**
...
- [ ] **Step 4: 失敗テストを書く（readiness gate × 3 fields）**
...
- [ ] **Step 5: テスト赤確認 → `ClaimInputRequirementProvider` に readiness ルール追加**
...
- [ ] **Step 1: 失敗テストを書く（binding）**

3 フィールドの ObservableProperty binding test を追加。

- [ ] **Step 2: 実装（ViewModel + XAML）**
...
- [ ] **Step 5: negative test 追加 → テスト緑 + コミット**
...
- [ ] **Step 1: 失敗テストを書く（binding × 4 fields）**
...
- [ ] **Step 4: readiness rules 追加 + negative tests**
...
- [ ] **Step 1: 失敗テストを書く（binding × 3 fields）**
...
- [ ] **Step 1: 失敗テスト（binding + readiness）**
...
- [ ] **Step 3: `ClaimReportGenerator` 骨組み + `GenerateServiceProvisionRecord` 実装**
...
- [ ] **Step 5: DI で `IFileSaveService` と `GenerateClaimReportsUseCase` の解決確認**
...
- [ ] **Step 1: `./build/ci.sh` を実行して末尾出力を保存**

Run: `./build/ci.sh 2>&1 | tail -40`
Expected: `==> CI OK`

- [ ] **Step 2: `docs/phase3-2-acceptance.md` を作成**
...
- [ ] **Step 3: `docs/open-questions.md` の該当 21 report フィールド関連項目を [x] へ**

対応する open-question 行（`Phase 3-1へ送る未実装入力` セクションと関連する 21 `report:*` フィールドをrefer するもの）を `[x]` に変え、末尾に `2026-07-20 クローズ / Phase 3-2` を追記。

- [ ] **Step 4: `docs/phase3-claim-field-mapping.md` の Phase 3-1 note を更新**
...
- [ ] **Step 6: `CHANGELOG.md` に Phase 3-2 エントリ追記**
...
- [ ] **Step 8: 本 plan の Task 15 チェックを入れて締める**
