...
embedded `sources.json`から2026-05と2026-06の異なる版を解決すること、ADR 0020の公式資料カタログ60件と3つのrelease束がID・順序を含めて完全一致することをテストする。`supersedes`／`corrects`／`supplements`の全参照先もカタログ内に存在しなければならない。重複・必須文字列の空白・未知source・不正SHA・schema version不一致、property名の大文字小文字違い、重複property、required propertyのnull、未知propertyを個別に拒否する。nullは`publishedAt`、`effectiveTo`、関係配列、注記などschemaでnullableとしたpropertyだけで許可する。

さらに6つのmaster file全てについて、ファイル名と`masterKind`の不一致、未知`masterKind`、期待ファイル欠落、重複key＋effectiveFrom、逆転期間、同一keyの期間重複、同一keyの期間空白、最後が有限期間で終わる暗黙の将来空白、未知source、空`values`（entryが存在する場合）、top-level／entryの未知propertyを個別に拒否する。`values` object内はPhase 3-1で各masterKindのschemaを具体化する拡張点なので、Task 11ではproperty名を閉じず、objectであること、entryが存在する場合に空でないことだけを固定する。

2つのschema resourceは、embedded resourceとして存在し、JSONとしてparse可能で、Task 11のrequired、nullable、`additionalProperties: false`、6種類の`masterKind`を表していることをテストする。Task 11では新しいJSON Schema engine packageを追加せず、実行時の正本をstrict DTO deserializeと後段semantic validator、schema JSONを同じ契約を表す配布仕様とする。
...
`sources.json`にはADR 0020の公式資料カタログ60件を、`documentId`、`title`、`publisher`、`effectiveAt`、`publishedAt`、`retrievedAt`、`url`、`sha256`、`supersedes`、`corrects`、`supplements`、`applicabilityNote`、`correctionNote`を省略せず入れる。動的入口ページのlocator evidenceと、B型マスタ入力に使わないサービスコード分割1もカタログには保持するが、releaseの`sourceDocumentIds`にはADR 0020の閉じた3束だけを入れる。

Infrastructure内部DTOは関係配列と注記を完全な形で保持・検証する。現行Domainの`ClaimSourceDocument`は単一`Supersedes`と`Notes`だけなので、`supersedes`が0件又は1件であることを確認して`ClaimMasterCatalogPolicy`検証用へ投影し、`corrects`／`supplements`をDomainへ誤って`Supersedes`として潰さない。全関係の完全性検証は投影前のInfrastructure DTOに対して行う。

各マスタファイルは次の形で作り、3-0では`entries`を空にする。
...
`LoadEmbedded()`とテスト用`internal Load(Stream sources, IReadOnlyDictionary<string, Stream> masters)`を持たせる。`System.Text.Json`のweb defaultsを基にしても既定のcase-insensitiveを使わず、既存`CsvSpecificationLoader`と同様に`PropertyNameCaseInsensitive = false`、`AllowDuplicateProperties = false`、`RespectNullableAnnotations = true`、`RespectRequiredConstructorParameters = true`、`UnmappedMemberHandling = Disallow`を明示する。top-level／source／release／entryのproperty名の大文字小文字違い、未知property、契約外nullを拒否する。deserialize後にInfrastructure DTO上の全参照を検証し、Domainへ投影した後に`ClaimMasterCatalogPolicy.Validate`を必ず通す。
...
期待するファイル名→`masterKind`を`basic-rewards.json`→`basic-rewards`、`additions.json`→`additions`、`region-unit-prices.json`→`region-unit-prices`、`burden-caps.json`→`burden-caps`、`transition-rules.json`→`transition-rules`、`service-codes.json`→`service-codes`の完全一致で固定する。各entryの`key`、inclusive適用期間、source参照、`values` object、同一keyの期間重複・期間空白・有限の最終期間を検証する。

Task 11では`Unavailable` discriminatorをschemaへ推測追加しない。通常entryだけでは表せない非適用期間又は廃止後期間が現れた場合はfail closedとし、Phase 3-1計画でJSON discriminatorとDomain表現を確定するまで受理しない。3-0の空`entries`は有効だが、entryが1件でもある場合の空`values`は無効とする。provider constructorは版解決だけを行う場合でもvalidatorを必ず実行し、不正master fileを未使用のまま見逃さない。

- [ ] **Step 6: resourceとDIを配線する**

Infrastructure csprojへ`ClaimMasters/Schema/*.json`と`ClaimMasters/Seed/*.json`をEmbeddedResource登録する。`LoadEmbedded()`はschema 2件、`sources.json`、6 master filesをexact filenameで開き、欠落・重複resourceを拒否する。DIは遅延生成の`AddSingleton<IClaimMasterProvider, JsonClaimMasterProvider>()`にせず、登録処理中に全resourceを検証してからinstance登録する。
