...
| 12か月未満、休止その他の短期実績 | 登録済み一次資料には、通常式へ休止期間を0で補完する規則も、実開所月数で除す正式な短期再計算式もない | 通常式で0埋め、年換算、実開所月数除算又は近接年度補完をしない。適用できる新規指定・経過措置の例外について、指定権者確認済みの届出平均工賃値、届出区分、対象期間及び根拠書類が全て揃う場合だけその専用分岐で扱い、それ以外は算定不能とする | 製品安全判断。公式に明記された例外期間は `r6-calculation-note`物理291頁、`r8-calculation-note`物理292〜293頁 |
| 対象年度の開所日又は延べ利用者が0 | 通常式では0除算となる | 0円、区分8又は届出区分を生成せず算定不能とする | 製品安全判断 |
| 年度途中を含む新規指定 | 初年度の1年間は平均工賃月額1万円未満とみなす。年度途中指定は初年度及び2年度目の1年間が対象。支援開始から6月経過した月から当該年度3月までは、開始後6月間の平均工賃月額に応じる届出を選択できる | 公式の6月値を通常式の12から推測せず、次段落の届出済み値・区分・根拠を必須にする | `r6-calculation-note`物理291頁、`r8-calculation-note`物理292〜293頁 |
...
`lowerInclusiveYen` は下端を含み、`upperExclusiveYen` は上端を含まない。`null` の上端は上限なし、上下端がともに`null`の行は平均工賃額から選択できない届出専用行であり、金額境界を評価する前に公式届出と経過措置根拠を直接検証して選ぶ。負の平均工賃月額は全ての数値行に該当しない。
...
`r8-fee-notice`物理43〜55頁と `r8-service-codes-2-xlsx` のワークブック順38「18就労継続支援(B・基本)」7〜2220行でも、改定対象外、変更なし及び改定対象の数値境界を定員・人員配置別に照合した。`r8.filed-transition.9` は上下端を持たず、平均工賃月額から生成せず、公式届出区分9と経過措置根拠を直接検証する。
...
- `r8.filed-transition.9` は、2026-06以降の対象サービス月、指定権者が受理した当月の公式届出区分9、対象条件、過去区分及び必要な根拠書類が全て一致する場合だけ直接選択する。平均工賃の計算又は数値区分の検索失敗時のフォールバックにしない（`r8-b-reward-band-guide`物理5〜7頁、`r8-qa-v1`物理14〜15頁）。
...
2. `OfficeClaimProfile`の当月届出、受理状態及び経過措置判定を検証する。2026-06以降で`FiledTransitionOnly`の場合は、当月の公式届出区分が9であること、対象サービス月、対象条件、過去区分及び必要な根拠書類が全て一致することを先に直接検証する。一致すれば金額を計算・評価せず `r8.filed-transition.9` を選んで手順7へ進み、不足又は不一致なら停止する（`r8-b-reward-band-guide`物理5〜7頁、`r8-qa-v1`物理14〜15頁）。
3. 区分9以外だけ、`AverageWageCalculator`で `baseAverageWageMonthYen` を算出する。新規指定6月値を選んだ場合だけ、指定権者確認済みの届出値を別入力として使用する。
4. 重度者支援体制加算(Ⅰ)を算定し、2,000円加算後の値を基本報酬選択へ使う届出がある場合は、基礎値を上書きせず `adjustedAverageWageForBasicRewardYen = baseAverageWageMonthYen + 2,000` を別値として作る。選択根拠がない場合は基礎値を使う（`r6-calculation-note`物理289頁、`r8-calculation-note`物理290頁）。
5. 2026-05以前は `r6.standard.*`、2026-06以降は指定権者確認済みの `ReformTarget`、`ReformExempt`又は`UnchangedBelow15000`に対応する数値区分群だけを開く。`Unknown`は拒否し、`FiledTransitionOnly`をこの金額境界分岐へ入れない。
6. 開いた数値区分群の金額境界から期待区分を1件求め、`OfficeClaimProfile`の当月届出区分と一致することを検証する。届出が未受理、複数、期間外又は不一致なら拒否する。
7. 検証済みの届出済み報酬体系、人員配置、定員規模、区分keyを請求マスタへ渡してサービスコード・単位数を解決する。平均工賃計算器はサービスコードを返さない。
...
| 区分9 | 2026-06より前、公式届出が区分9でない、未受理・複数、対象条件・過去区分・必要書類・サービス月の欠落又は不一致、金額境界から生成している |
| 数値区分の当月届出 | 未届、未受理、複数、サービス月の期間外、算出平均と金額境界から求めた期待区分との不一致 |
