...
- **副産物として実在のレジストリ不整合が判明した**: `r8-grant-decision-administration-202606` は
  CSV 側が厚労省 PDF（現在 404・historical としてバイト列保持）、claim-master 側が北九州市のミラーを
  指しており、**同じ ID で別ファイルを検証している**。どちらを正本にするか（ID を分けるか）は
  `docs/open-questions.md` で追跡し、解消されたらテストが RED になるよう宣言してある。
