...
**行2266・2268について（最終レビューM-1・2026-07-26追記）**: 上記表が2265→2267、2267→2269と2箇所で行番号が飛んでいる理由を、`r8-service-codes-2-xlsx`を再取得し（`shasum -a 256`が`307b631ed91a...`でsources.json登録値と一致）、`openpyxl`で該当2行を直接確認した。行2266・2268はいずれも列V（追加条件欄）に「指定障害者支援施設において行った場合」、列AR（算定単位欄）に「単位加算」を持つが、列A（種類）・列B（項目）・列C（サービス内容略称）が空欄でサービスコードが割り当てられていない。すなわち両行は(Ⅱ)イ・(Ⅱ)ロの施設variant用に予約された書式上のプレースホルダ行であり、コードは存在しない。これは率表（本ADR「抽出方式と2方式の一致確認結果」節）の「（施設別立てなし）」という記述と一致し、この2行は「施設variant未投入」ギャップの内側（未投入のコードが存在する）ではなく外側（投入すべきコード自体が存在しない）であることが一次資料から直接確認できた。
...
**追記（2026-07-26、Phase 3-5 / ADR 0047）**: 上表の障害者支援施設variant行は、Phase 3-5でADR 0047により一次資料から一意に確定できた4区分（465138・465176・465140・465141）を投入し解消した。施設区分を入力した事業所が正しい率・コードへ解決するようになった。詳細は[ADR 0047](0047-r8-designated-support-facility-variants.md)と`docs/phase3-5-acceptance.md`を参照。処遇改善(Ⅴ)は本追記時点でも未確定のまま。
