...
3. **保存は `ClaimInput` の子**（`ClaimInputGenericValue`）。独立した履歴を持たず、訂正は親の
   ...
   **親にナビゲーションは張らない**（明細行と同じ片方向 FK）。リポジトリが別クエリで読んで詰め替え、
   書き込みでも明示的に追加する。ナビゲーションを張ると EF の relationship fix-up が
   `IReadOnlyList` の不変コレクションへ子要素を追加しようとして**読込が落ちる**（Codex 指摘 P1。
   `ClaimInputRepositoryTests.Generic_pass_through_values_round_trip_and_stay_empty_on_cancel` で固定）。
   record の値セマンティクスを守るため、可変コレクションを Domain へ持ち込む方は採らない。
4. **型・桁数の検証は仕様を所有する層**（`IClaimGenericFieldCatalog.ValidateValue`）。
   Application に `dataType` の語彙を持ち込まない（境界ガードが実際に落ちた）。
   出力時は encoder が同じ規則で再検証する（二重の網）。日付・年月は **CSV へ出る形**
   （`yyyyMMdd` / `yyyyMM`）で受ける。区切り記号つきで受けると項目定義の桁数（8/6）を必ず超え、
   超えなかったとしても encoder の「数字のみ」検査で落ちるため、宣言できても永久に出力できない
   項目になる（Codex 指摘 P1）。検証失敗は `ClaimGenericValueInvalidException` →
   `ClaimInputSaveException(InvalidValue)` に変換し、**利用者向けの理由を保ったまま**表示する
   （履歴競合として再読込すると入力が捨てられる。Codex 指摘 P2）。
5. **範囲は月次スコープのみ**で、**宣言時に機械判定する**。行スコープ表
   （`CsvRecordRowScopes`）は行の発生計画と宣言可否の判定が同じ表を引き、受給者×サービス提供年月で
   1 行のレコード以外は読み込みで拒否する。日ごと明細に宣言すると同じ値が全日へ複製され、
   ファイル単位のレコードでは受給者行が無く生成が落ちる（Codex 指摘 P1）。
   同じ理由で `uiSurface` も実装済みの `ClaimInputView` に限る。
   日次スコープの新規項目は当面型付き列で追加する。
6. **PreviewHash に含める**（名前昇順）。含めないとプレビュー後に書き換えても同じ hash で確定できる。
7. **同じ値を複数の項目が運ぶ宣言は 1 欄にまとめる**（`FieldIds` を持つ 1 宣言にする）。
   見せ方・型・桁数が食い違う同名宣言は読み込みで拒否する。まとめないと同名の欄が 2 つ並び、
   保存時に名前をキーにした辞書が重複で落ちる（Codex 指摘 P2）。
...
- 月次スコープ外のレコード・未実装の画面に宣言すると**読み込みで落ちる**
- 日付/年月は CSV 書式（8桁/6桁）でのみ通り、区切り記号つきは拒否される
...
- Codex レビュー（P1×3・P2×2）を全件取り込み、いずれも回帰テストで固定した。
