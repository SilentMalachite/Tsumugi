...
- **DB テーブル**: `WageAdjustments`（`WageAdjustmentConfiguration` で `(OfficeId, RecipientId, YearMonth, Type) WHERE Kind = 1` の partial unique index を設定し、同一利用者×月×種別の New を DB レベルで一意化する。Correction/Cancel はこの制約の対象外。同一月に異なる Type が複数存在するのは正当）
