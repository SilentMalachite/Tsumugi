...
public sealed partial class SpecEvidenceLedgerTests
{
    ...
    public void Every_citation_that_points_at_an_item_row_matches_the_mechanical_extraction()
    {
        ...
        var pages = PagesFromExtractions();
    ...
    public void Every_generator_rule_anchor_matches_the_mechanical_extraction()
    {
        ...
        // 共通編の抽出（ADR 0037 追記）で、事業所編・共通編の両方の source アンカーが検証対象になった。
        verified.Should().Be(372, "全 generatorRule の source アンカーを抽出結果と突合できる");
    }
    ...
    public void Known_evidence_gaps_are_declared_and_only_shrink()
    {
        ...
        // 共通編の抽出対応（ADR 0037 追記）で 1 件クローズしたため上限を締める。
        Catalog.EvidenceGaps.Should().HaveCountLessThanOrEqualTo(2);
    }
    ...
    /// <summary>checked-in の抽出結果（事業所編・共通編）。</summary>
    private static readonly string[] ExtractionFiles =
        ["provider-r7-10-item-tables.json", "common-r7-10-item-tables.json"];

    /// <summary>登録済みの抽出結果を (レコード, 項番) → 頁 に畳む。</summary>
    private static Dictionary<(string RecordId, int Position), int> PagesFromExtractions() =>
        ExtractionFiles
            .Select(LoadExtraction)
            .SelectMany(extraction => extraction.Records)
    ...
    private static ItemTableExtractionFile LoadExtraction(string fileName)
    {
